    ...
    public class DependsOn1<D> {
        ...
        public Spec<T> populate(BiConsumer<PropertySpec<T>, D> definition) {
            spec.appendStructureDefinition(new PropertyStructureDefinition<>(singletonList(dependent), property,
                    values -> condition.test((D) values.get(0)),
                    (ps, values) -> definition.accept(ps, (D) values.get(0))));
            return spec;
        }
    }
    ...
    public class DependsOn2<D1, D2> {
        ...
        public Spec<T> populate(TriConsumer<PropertySpec<T>, D1, D2> definition) {
            spec.appendStructureDefinition(new PropertyStructureDefinition<>(asList(dependent1, dependent2), property,
                    values -> condition.test((D1) values.get(0), (D2) values.get(1)),
                    (ps, values) -> definition.accept(ps, (D1) values.get(0), (D2) values.get(1))));
            return spec;
        }
    }
}
...
class PropertyStructureDefinition<T> {
    ...
    final BiConsumer<PropertySpec<T>, List<Object>> definition;
    final Predicate<List<Object>> condition;

    PropertyStructureDefinition(List<String> dependent1, String property, Predicate<List<Object>> condition, BiConsumer<PropertySpec<T>, List<Object>> definition) {
    ...
    void apply(Spec<T> spec, ObjectProducer<?> rootProducer) {
        List<Object> dependents = this.dependents.stream().map(rootProducer::descendantForRead).map(Producer::getValue).collect(toList());
